...
In order to isolate the state between different projects, you can either work within different configuration directories or make use of the **workspaces** which automatically keep state under a subdirectory named `terraform.tfstate.d/workspace_name` located within the same configuration directory. Using workspaces is one way to follow the DRY pattern and avoid repeating code as you can use `terraform.workspace` within your configuration code to differentiate your configuration according to the current workspace you are using. From my current understanding I would use **modules** instead of workspaces but I guess there is a use case for workspaces also. I find also the idea of workspaces very error prone since it relies to me remembering to switch to the correct workplace prior to doing my job, which for me is a bad idea. 
